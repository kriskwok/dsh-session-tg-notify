...
先看宿主记录，它会直接告诉你**有没有下发过帧、给了谁、下的是什么帧**：

```sh
curl -s http://127.0.0.1:3080/session-notify/state | python3 -m json.tool
```

看 `recentFrames`（最近 20 次下发）与 `presence`：

| 现象 | 含义 |
|---|---|
| `recentFrames` 为空 | 事件没走到投递环节。检查日志里有没有「被时长过滤丢弃」 |
| 有记录、事件是 `toast` | 判定为前台，走页面内提示（APP 在前台时这是预期行为） |
| 有记录、事件是 `webnotify` 但没弹 | 帧已下发，是系统层面没显示 → 查系统通知权限 |
| `channel` 是 `dropped` | 写 SSE 失败，连接已断（页面重载/被系统回收） |
| `presence` 是 `offline` | 没有任何客户端在线 → 改走 Telegram，桌面通道不适用 |

两个曾经导致「收不到」的坑，已在 0.1.6 修掉：

- **同 tag 静默替换**：早期通知 `tag` 只用事件类型，macOS 会把同类型的新通知**静默替换**通知中心里还挂着的那条（不弹横幅、不响铃）。现在 `tag` 拼上 `sessionId + 事件类型 + 宿主时间戳 + 客户端序号`，每条都是独立条目。
- **短 turn 被静默丢弃**：`minDuration` 早先默认 10 秒且**静默 return**，导致「发个『你好』这种几秒结束的 turn」既收不到通知、也没有任何日志，看起来像插件坏了。现在默认 0（不过滤）、过滤时打日志、设置面板可调。
