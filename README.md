...
同一份代码同时用于 **Web**（浏览器）、**桌面版**（Electron）与 **headless**，无需按 profile 分叉——判定源和数据通路在三端一致。下表是实测结论：

| 能力 | 机制 | Web | 桌面版 | headless |
|---|---|---|---|---|
| APP/页面内 toast | 往 `document.body` 插 DOM | ✅ | ✅ | — |
| 系统通知 | 页面侧 `new Notification()` | ✅ | ✅（需系统已授权） | — |
| 前台/后台判定 | `document.visibilityState` + `document.hasFocus()` | ✅ | ✅ | — |
| 锁屏判定 | 宿主 `ioreg` 读 `CGSSessionScreenIsLocked` | ✅ | ✅ | ✅ |

- **桌面版不需要特殊适配。** Electron 单窗口常见问题（`document.hasFocus()` 恒为 `true` 导致永远判不出后台）在 DSH 桌面版上**不存在**：实测连续切换到 Finder / TextEdit / Safari，每次都正确翻成 `background`，且不会自抢焦点。
- **系统通知由页面发出**这一条对桌面版同样成立——Electron 渲染进程有完整 DOM，主进程也带 `setPermissionRequestHandler`（会处理通知权限请求）。
- **锁屏判定走宿主**，与客户端无关，所以「网页后台且锁屏时推送」那一档在桌面版照常生效。
- **桌面版只发系统通知的条件就是「APP 不在前台」**，不做前后台 toast 区分——窗口在前台时走页面内提示，失焦时走系统通知。
- `headless` 没有客户端，SSE 恒无连接 → 始终判为「离线」，因此只走 Telegram。

如果桌面版首次收不到系统通知，先确认 macOS **系统设置 → 通知 → DeepSeek Harness** 已允许；未授权时客户端会静默降级为页面内提示。
...
- **锁屏探测只在 macOS 宿主上可用。** DSH 若跑在 Linux 服务器（如远程 Agent 服务器），宿主读不到你的屏幕锁屏状态，`screen.supported` 为 `false`，此时「网页后台且锁屏时推送」这一档**永不触发**（后台仍发系统通知，网页关闭仍走 Telegram）。宿主与你的机器不在同一台时，可改用 `DSH_SESSION_TG_NOTIFY_SCREENLOCK=off` 显式关闭该档，避免误以为它在工作。
